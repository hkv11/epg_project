...
adrn = kocak_ge[kocak_ge["AM_class_stringent"] == "ADRN"]
mes  = kocak_ge[kocak_ge["AM_class_stringent"] == "MES"]

print(f"ADRN tumors: {adrn.shape[0]}")
print(f"MES tumors:  {mes.shape[0]}")
print(f"HYBRID excluded: {(kocak_ge['AM_class_stringent'] == 'HYBRID').sum()}")
...
gene_cols = [col for col in kocak_ge.columns if col not in meta_cols]
...
results_df.to_csv("results/kocak_GE_differential.csv", index=False)
print("\nResults saved to results/kocak_GE_differential.csv")