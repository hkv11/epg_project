...
# ─────────────────────────────────────────────
# Boxplot — KOCAK top ADRN and MES TFs side by side
# Run from project root: python plotting/plot_kocak_boxplot.py
# ─────────────────────────────────────────────
...
tf_df    = pd.read_csv("results/kocak_TF_differential.csv")
datasets = load_all()
...
def draw_box(ax, tf_name, q_val, title):
    plot_df = kocak_tf[["condition", "AM_class_stringent", tf_name]].dropna()
...
fig.suptitle("KOCAK — Top Differentially Active TFs (ADRN vs MES)",
             fontsize=13, fontweight="bold", y=1.01)

plt.tight_layout()
plt.savefig("results/plots/kocak_boxplot_TF.png", dpi=300, bbox_inches="tight")
plt.close()
print("Saved: results/plots/kocak_boxplot_TF.png")